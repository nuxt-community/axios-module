<% if (options.polyfillFetch === true) { %>import 'whatwg-fetch'<% } %>
...
const createAxiosInstance = axiosOptions => {
  <% if (options.retry === true) { %>axiosOptions.retry = true<% } %>
<% if (typeof options.retry === 'object') { %>axiosOptions.retryConfig = <%= serialize(options.retry) %> <% } %>

  // Create new axios instance
  const axios = Axios.create({
    ...axiosOptions,
    // do not set headers on initialization, as they have a differnet form in the module
    // than in (h)axios itself,
    // we are setting them already onRequest
    headers: {}
  })
  axios.CancelToken = Axios.CancelToken
  axios.isCancel = Axios.isCancel

  axios.defaults = axiosOptions;
