...
const createAxiosInstance = axiosOptions => {
  ...
  axios.onRequest((config) => {
    config.headers = { ...config.defaultHeaders, ...config.headers }
  })
...
export default (ctx, inject) => {
  ...
  const axiosOptions = {
    defaultHeaders: <%= JSON.stringify(options.headers, null, 4) %>,
    baseURL,
    headers: {}
  }
  ...
  if (process.server && ctx.req && ctx.req.headers) {
    ...
    axiosOptions.defaultHeaders = { ...axiosOptions.defaultHeaders, ...reqHeaders }
  }
  <% } %>

  if (process.server) {
    // Don't accept brotli encoding because Node can't parse it
    axiosOptions.defaultHeaders['accept-encoding'] = 'gzip, deflate'
  }
